...
#define posix_print(format, ...) __android_log_print(ANDROID_LOG_INFO, "sleep", format, ##__VA_ARGS__)
...
Android_VideoInit(_THIS)
{
	...
	posix_print("Android_VideoInit------(%ix%i)\n", Android_ScreenWidth, Android_ScreenHeight);
...
Android_SetScreenResolution(int width, int height, Uint32 format, float rate)
{
	...
	posix_print("#%i, Android_SetScreenResolution------(%ix%i), format: 0x%08x, rate: %7.2f, Android_Window: %p\n", 
		times, height, width, format, rate, Android_Window);
