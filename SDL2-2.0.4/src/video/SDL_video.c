...
extern void SDL_ThirdInit();
extern void SDL_ThirdQuit();
extern void SDL_PeripheralInit();
extern void SDL_PeripheralQuit();
...
extern int UIKit_MicrophonePermission();
extern void UIKit_StartRecordAudio(SDL_Window* window, const char* file);
extern void UIKit_StopRecordAudio(SDL_Window* window);
...
extern int UIKit_GetScale();
extern void UIKit_ShowStatusBar(SDL_bool show);
...
SDL_SetWindowData(SDL_Window * window, const char *name, void *userdata)
{
	...
	if (!window) {
		if (len == 5 && !SDL_memcmp(name, "scale", len)) {
#if defined(__IPHONEOS__) && __IPHONEOS__
			return (void*)(UIKit_GetScale());
#endif
			return (void*)(2);
		} else if (len == 14 && !SDL_memcmp(name, "battery_enable", len)) {
		...
		} else if (len == 14 && !SDL_memcmp(name, "statusbar_show", len)) {
			SDL_bool show = (int)userdata? SDL_TRUE: SDL_FALSE;
#if defined(__IPHONEOS__) && __IPHONEOS__
			UIKit_ShowStatusBar(show);
#endif
		} else if (len == 8 && !SDL_memcmp(name, "iexplore", len)) {
#if defined(__IPHONEOS__) && __IPHONEOS__
			UIKit_IExplore((const char*)userdata);
#endif
		} else if (len == 15 && !SDL_memcmp(name, "statusbar_style", len)) {
			SDL_bool white_fg = (int)userdata? SDL_TRUE: SDL_FALSE;
#if defined(__IPHONEOS__) && __IPHONEOS__
			UIKit_SetStatusBarStyle(white_fg);
#endif
			...
			return (void*)1;
#endif
		} else if (len == 13 && !SDL_memcmp(name, "mixable_audio", len)) {
...
#if defined(__IPHONEOS__) && __IPHONEOS__
			if (len > 2) {
				UIKit_StartRecordAudio(window, name + 2);
			} else {
				UIKit_StopRecordAudio(window);
			}
#endif
			return NULL;
		} else if (len >= 3 && name[0] == '#' && name[1] == '#' && (name[2] == 'G' || name[2] == 'A')) {
