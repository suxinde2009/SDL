...
- (void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral2 advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI
{
	...
	if (!peripheral->cookie) {
        ...
        if ([service_uuids count]) {
            const char* uuid = [[[service_uuids objectAtIndex: 0] UUIDString] UTF8String];
            size_t s = SDL_strlen(uuid);
            peripheral->uuid = (char*)SDL_malloc(s + 1);
            memcpy(peripheral->uuid, uuid, s);
            peripheral->uuid[s] = '\0';
        }
        
		// if kCBAdvDataManufacturerData doesn't exist, mac_addr will return NULL;
...
void UIKit_DisconnectPeripheral(const SDL_BlePeripheral* peripheral)
{
	CBPeripheral *data = (__bridge CBPeripheral *)peripheral->cookie;
	[ble->bleManager cancelPeripheralConnection:(CBPeripheral*)data];
}

void UIKit_GetServices(const SDL_BlePeripheral* peripheral)
{
	CBPeripheral *data = (__bridge CBPeripheral *)peripheral->cookie;
...
void UIKit_ReadCharacteristic(const SDL_BlePeripheral* peripheral, const SDL_BleCharacteristic* characteristic)
{
	CBPeripheral* peripheral2 = (__bridge CBPeripheral *)peripheral->cookie;
...
void UIKit_SetNotify(const SDL_BlePeripheral* peripheral, const SDL_BleCharacteristic* characteristic)
{
	CBPeripheral* peripheral2 = (__bridge CBPeripheral *)peripheral->cookie;
	CBCharacteristic* characteristic2 = (__bridge CBCharacteristic *)characteristic->cookie;
...
void UIKit_WriteCharacteristic(const SDL_BlePeripheral* peripheral, const SDL_BleCharacteristic* characteristic, const unsigned char* data, int size)
{
	CBPeripheral* peripheral2 = (__bridge CBPeripheral *)peripheral->cookie;
	CBCharacteristic* characteristic2 = (__bridge CBCharacteristic *)characteristic->cookie;
