...
void android_handle_bledid(SDL_bool foreground)
{
	...
	if (bledids_rd == -1) {
		posix_print("android_handle_bledid, bledids_rd == -1\n");
...
{
	if (didbackground) {
		android_handle_bledid(SDL_FALSE);
	}
}
...
static void Android_DisconnectPeripheral(const SDL_BlePeripheral* peripheral)
{
	jobject gatt = peripheral->cookie;
	if (!gatt) {
		return;
...
static void Android_GetServices(const SDL_BlePeripheral* peripheral)
{
	jobject gatt = peripheral->cookie;
	if (!gatt) {
		return;
...
static void Android_ReadCharacteristic(const SDL_BlePeripheral* peripheral, const SDL_BleCharacteristic* characteristic)
{
	jobject gatt = peripheral->cookie;
	if (!gatt) {
		return;
...
static void Android_SetNotify(const SDL_BlePeripheral* peripheral, const SDL_BleCharacteristic* characteristic)
{
	jobject gatt = peripheral->cookie;
	if (!gatt) {
		return;
...
static void Android_WriteCharacteristic(const SDL_BlePeripheral* peripheral, const SDL_BleCharacteristic* characteristic, const unsigned char* data, int size)
{
	jobject gatt = peripheral->cookie;
	if (!gatt) {
		return;
...
static void Android_ReleaseCharacteristicCookie(const SDL_BleCharacteristic* characteristic)
{
...
static SDL_MiniBle* Android_CreateBle()
{
	...
	ble->SetNotify = Android_SetNotify;
